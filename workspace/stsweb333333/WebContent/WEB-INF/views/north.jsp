     ...
     var Html = [];
		...
		if(area=='south'){
			for (var i = 100; i < 118; i++) {
				Html[i]="<li><a class=location href=north.do?map_num="+i+">"+south[i-100]+"</a></li>"
		    ...
		    Html = [];
		 	...
		 	for (var j = 118; j < 142; j++) {
		      Html[j]="<li><a class=location href=north.do?map_num="+j+">"+north[j-118]+"</a></li>"
		     $("#map").append(Html.join(""));
		         Html = [];
		 	 }
		}
...
<body>
  <section>
    <div  class="container" id="left-nav">
      <!-- 좌측 지역 nav -->
      <div class="seoul">
        <button id="bu" class="map_gb on" onclick="area('north',${map_num})">
        </button>
        <button id="bu" class="map_gn" onclick="area('south',${map_num})" >
        </button>
      </div>
